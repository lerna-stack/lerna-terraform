...
Source0: https://www.haproxy.org/download/2.0/src/devel/%{name}-%{version}.tar.gz
...
%build
%{__make} USE_PCRE=1 DEBUG="" ARCH=%{_target_cpu} TARGET=linux-glibc USE_OPENSSL=1
...
%{__install} -c ../../SOURCES/%{name}-%{version}/%{name}.service %{buildroot}%{_unitdir}/%{name}.service
