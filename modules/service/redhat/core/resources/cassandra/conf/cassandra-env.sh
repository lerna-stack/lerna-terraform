# cassandraをリモートでアクセスできるように設定変更するために本ファイルでオリジナルを上書きする
...
# for Recovery operation when node down.
# HOST_IP is down node IP address.
#HOST_IP=
#JVM_OPTS="$JVM_OPTS -Dcassandra.replace_address=$HOST_IP"
